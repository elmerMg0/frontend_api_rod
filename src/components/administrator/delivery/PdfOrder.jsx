...
import { typeTicketExists } from "./DeliveryApp";
...
const PdfOrder = ({ infoSale, setShowViewer, updateState, items, typeTicket}) => {
  ...
  const handleCancel = () => {
    setShowViewer(false);
    updateState(saleState.ENVIADO, infoSale.id)
  }
          ...
          <Document>
            ...
            <Page size='A4'>
              ...
              <View style={styles.sectionHeader}>
                ...
                <Text>Cliente: {infoSale.nombre}</Text>
                <Text>Tipo Entrega: {infoSale.tipo_entrega}</Text>
                {
                  typeTicket === typeTicketExists.DELIVERY &&
                  <>
                  <Text>Tipo Entrega: {infoSale.nombre}</Text>
                  <Text>Tipo Entrega: {infoSale.telefono}</Text>
                  <Text>Tipo Entrega: {infoSale.direccion}</Text>
                  <Text>Tipo Entrega: {infoSale.descripcion_direccion}</Text>
                  <Text>Tipo Entrega: {infoSale.hora}</Text>
                  </>
                }
              </View>
